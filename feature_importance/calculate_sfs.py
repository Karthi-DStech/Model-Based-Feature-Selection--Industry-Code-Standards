    ...
    def perform_sfs(self, X_train, y_train, model_params=None, tuning_phase=None):
        ...
        if self._opt.sequential_feature_selector == True:
            ...
            try:
                ...
                self.logger.update_log(
                    f"SFS {phase}", "Verbose", self._opt.sfs_verbose
                )
                
                self.logger.update_log(
                    f"SFS {phase}", "Scoring", self._opt.sfs_scoring
                )
                ...
                self.logger.update_log(
                    f"SFS {phase}", "Number of features selected", len(selected_features)
                )
